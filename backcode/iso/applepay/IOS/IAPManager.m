...
-(void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response{
    ...
    for (SKProduct *p in products) {
        UnitySendMessage("Main Camera", "ShowProductList", [[self productInfo:p] UTF8String]);
    }
...
-(void) provideContent:(SKPaymentTransaction *)transaction{
    UnitySendMessage("Main Camera", "ProvideContent", [[self transactionInfo:transaction] UTF8String]);
}
...
-(void) completeTransaction:(SKPaymentTransaction *)transaction{
	//支付完成后回调Main Camera节点上ApplePlayMgr脚本的SetAppPayId接口和ProvideContent接口
    NSLog(@"Comblete transaction : %@",transaction.transactionIdentifier);
	 UnitySendMessage("Main Camera", "SetAppPayId", [transaction.transactionIdentifier UTF8String]);
...
-(void) failedTransaction:(SKPaymentTransaction *)transaction{
    NSLog(@"Failed transaction : %@",transaction.transactionIdentifier);
     UnitySendMessage("Main Camera", "PayFail","");
