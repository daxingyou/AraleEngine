...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class FastZip
	{
		...
		void ExtractFileEntryByWanghuan(ZipEntry entry, string targetName)
		{
			if ( !continueRunning_ )return;
			try {
				using ( FileStream outputStream = File.Create(targetName) ) {
					if ( buffer_ == null ) {
						buffer_ = new byte[4096];
					...
					while (copying) {
						int bytesRead = source.Read(buffer_, 0, buffer_.Length);
						if (bytesRead > 0) {
							outputStream.Write(buffer_, 0, bytesRead);
							mUnzipBytes+=bytesRead;
							mUpdateBytes+=bytesRead;
							if(mUpdateBytes>=1024*1024)
							{
								mUpdateBytes=0;
								mCallback(0.99f*mUnzipBytes/mUnzipSize, 0);
							}
						}
						else {
							outputStream.Flush();
                ...
                //LogManager.GetInstance().LogException(null, ex);
				mCallback(0.99f*mUnzipBytes/mUnzipSize, 1);
				continueRunning_ = false;
				throw;
			}
		}
		...
		void ExtractEntry(ZipEntry entry)
		{
			...
			if ( doExtraction && entry.IsFile ) {
				...
				ExtractFileEntryByWanghuan(entry, targetName);
			}
		}
