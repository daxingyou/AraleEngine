...
namespace LuaInterface
{
	...
	public class LuaStatic
	{
		...
		public static int loader(IntPtr L)
		{
			...
			LuaDLL.luaL_loadbuffer(L, file.text, file.bytes.Length, fileName);*/
			int bytes = 0;
			string str = LuaRoot.readLuaStringFromFile (fileName, out bytes);
			if (str == null)return 0;
			LuaDLL.luaL_loadbuffer(L, str, bytes, fileName);
			return 1;
		}
		...
		public static int dofile(IntPtr L)
		{
			...
			if( LuaDLL.luaL_loadbuffer(L, file.text, file.bytes.Length, fileName) == 0 )
			{
