...
public class ScaleCtrl : TCtrl {
	void OnPostRender()
	{
		...
		GL.MultMatrix(m*Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0,0,0), Vector3.one));
		drawTrig (mSel==SelType.YZ?Color.yellow:Color.red);
		GL.MultMatrix(m*Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0,0,-90), Vector3.one));
		drawTrig (mSel==SelType.XZ?Color.yellow:Color.green);
		GL.MultMatrix(m*Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0,90,0), Vector3.one));
		drawTrig (mSel==SelType.XY?Color.yellow:Color.blue);

		GL.MultMatrix (m);
		drawXYZTrig (mSel==SelType.XYZ?Color.yellow:Color.magenta);
		GL.PopMatrix ();
	}
	...
	void drawTrig(Color clr)
	{
		GL.Begin (GL.LINES);
		GL.Color (clr);
		GL.Vertex3 (0, 0.3f*mR, 0);
		GL.Vertex3 (0, 0, 0.3f*mR);
		GL.Vertex3 (0, 0.5f*mR, 0);
		GL.Vertex3 (0, 0, 0.5f*mR);
		GL.End ();
	...
	void drawXYZTrig(Color clr)
	{
		GL.Begin (GL.TRIANGLES);
		clr.a = 0.3f;
		GL.Color (clr);
		GL.Vertex3 (0.3f*mR, 0, 0);
		GL.Vertex3 (0, 0.3f*mR, 0);
		GL.Vertex3 (0, 0, 0.3f*mR);
		GL.End ();
	}
