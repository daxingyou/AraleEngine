...
Shader "TestShader/海浪" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			//水面
			fixed4 waterColor = tex2D(_GTex, float2(min(_Range.y, deltaDepth)/_Range.y,0));
			...
			//浪花
			half water_A = 1-min(_Range.z, deltaDepth)/_Range.z;//浅水区
			half water_B = min(_Range.w, deltaDepth)/_Range.w;  //深水区
